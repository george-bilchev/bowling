 ...
 * Each game has 10 frames by default, but 2 bonus frames are awarded if last frame was a
 * strike/spare. So, a game can have a maximum of 12 frames and a minimum of 10.
 */
public class BowlingGame {
      ...
      if (pins <= MAX_PINS_ROLLED) {
        ...
        if (isValidRoll(pins, attempt)) {
          ...
          if (attempt - 2 >= 0 && frames[attempt - 2].getBonus() > 0) {
            ...
            if (frames[attempt - 2].getBonus() == 0) {
              ...
              if (attempt - 3 >= 0) {
                ...
                frames[attempt - 2].setPoints(
                    frames[attempt - 3].getPoints() + frames[attempt - 2].getTemp());
              }
          ...
          if (attempt - 1 >= 0 && frames[attempt - 1].getBonus() > 0) {
            ...
            if (frames[attempt - 1].getBonus() == 0) {
              ...
              if (attempt - 2 >= 0) {
                ...
                // add points from prev frame to its points
                frames[attempt - 1].setPoints(
                    frames[attempt - 2].getPoints() + frames[attempt - 1].getTemp());
              }
              // this is the first frame
              else {
          ...
          if (frames[attempt].getFirstRoll() == EMPTY) {
            ...
            // required
            if (attempt == BONUS_FRAME_2 - 1
                && isStrike(LAST_FRAME - 1)
                && isStrike(BONUS_FRAME_1 - 1)) {
              attempt++;
            }
          }
          // second roll
            ...
            // open frame
              ...
              if (attempt - 1 >= 0) {
                frames[attempt].setPoints(
                    frames[attempt - 1].getPoints() + frames[attempt].getTemp());
              }
              // this is the first frame
              else {
  ...
  public boolean isValidRoll(int pins, int attempt) {
    ...
    if (frames[attempt].getFirstRoll() == EMPTY) {
      ...
      if (pins <= MAX_PINS_ROLLED) return true;
      else return false;
  ...
  public int getScore() {
    ...
    else return frames[ptr].getPoints();
  }
